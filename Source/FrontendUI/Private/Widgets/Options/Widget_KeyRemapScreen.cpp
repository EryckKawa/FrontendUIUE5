	...
	void ProcessPressedKey(const FKey& InPressedKey)
	{
		...
		case ECommonInputType::Gamepad:
			...
			if (CurrentInputType == ECommonInputType::Gamepad && InPressedKey == EKeys::LeftMouseButton)
			{
				...
				OnInputPreProcessorKeyPressed.
					ExecuteIfBound(InputActionData->GetDefaultGamepadInputTypeInfo().GetKey());
			...
			{
				OnInputPreProcessorKeySelectCanceled.ExecuteIfBound(
					TEXT("Detected non Gamepad Key pressed for Gamepad inputs. Key Remap has been canceled."));
