...
bool UListDataObject_KeyRemap::IsKeyAlreadyBoundToAnotherAction(const FKey& InKey, FString& OutConflictingActionName) const
{
	check(CachedOwningInputUserSettings);

	// Verifica se a chave já está sendo usada por outra ação
	for (const TPair<FGameplayTag, UEnhancedPlayerMappableKeyProfile*>& ProfilePair : CachedOwningInputUserSettings->GetAllSavedKeyProfiles())
	{
		...
		for (const TPair<FName, FKeyMappingRow>& MappingRowPair : MappableKeyProfile->GetPlayerMappingRows())
		{
			...
			{
				// Verifica se a chave é a mesma que queremos usar
				if (KeyMapping.GetCurrentKey() == InKey)
				{
					// Verifica se não é a mesma ação que estamos tentando remapear
					if (KeyMapping.GetMappingName() != CachedOwningMappingName || KeyMapping.GetSlot() != CachedOwningMappableKeySlot)
					{
						OutConflictingActionName = KeyMapping.GetDisplayName().ToString();
						return true;
					}
				}
			}
