...
void UWidget_ListEntry_KeyRemap::OnKeyToRemapPressed(const FKey& PressedKey)
{
	...
	FString ConflictingActionName;
	if (CachedOwningKeyRemapDataObject->IsKeyAlreadyBoundToAnotherAction(PressedKey, ConflictingActionName))
	{
		// Mostra uma tela de confirmação informando que a chave já está sendo usada
		UFrontendUISubsystem::Get(this)->PushConfirmScreenToModalStackAsync(
			EConfirmScreenType::Ok,
			FText::FromString(TEXT("Key Already Bound")),
			FText::FromString(FString::Printf(TEXT("Cannot remap because another action already uses this key: %s"), *ConflictingActionName)),
			[](EConfirmScreenButtonType ClickedButton){}
		);
		return;
	}

	// Se não há conflito, aplica o remapeamento
