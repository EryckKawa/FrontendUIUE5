...
TArray<UListDataObject_Base*> UOptionsDataRegistry::GetListSourceItemsBySelectedTabID(const FName& InSelectedTabID) const
{
	...
	for (UListDataObject_Base* ChildListData : FoundTabCollection->GetAllChildrenListData())
	{
		if (!ChildListData)
		{
			continue;
...
void UOptionsDataRegistry::FindChildListDataRecursively(UListDataObject_Base* InParentData,
	TArray<UListDataObject_Base*>& OutFoundChildData) const
{
	...
	for (UListDataObject_Base* SubChildListData : InParentData->GetAllChildrenListData())
	{
		if (!SubChildListData)
		{
			continue;
		...
		OutFoundChildData.Add(SubChildListData);

		if (SubChildListData->HasAnyChildListData())
		{
			FindChildListDataRecursively(SubChildListData, OutFoundChildData);
		}
	}
}
...
void UOptionsDataRegistry::InitEasterEggCollectionTab()
{
	UListDataObject_Collection* EasterEggTabCollection = NewObject<UListDataObject_Collection>();
	EasterEggTabCollection->SetDataID(FName("EasterEggTabCollection"));
	EasterEggTabCollection->SetDataDisplayName(FText::FromString("Easter Egg"));

	RegisteredOptionsTabCollections.Add(EasterEggTabCollection);
}
